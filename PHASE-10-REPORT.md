...
*Files: `src/config/navigation.ts`, `src/components/layout/Footer.astro`*
...
*Files: `src/components/navigation/SearchModal.astro`, `src/pages/search.astro`*
...
| 2 | Rank numerals (Popular Articles) failed AA | Used a *border* color (`--color-border-strong`) as text — border tokens aren't calibrated for text contrast | Switched to `--color-text-muted` |
...
| 8 | Affiliate/commerce CTA buttons rendered with invisible text when embedded in article content via MDX | Tailwind Typography's `.prose a { color }` selector was overriding the buttons' own `text-white`, because commerce components (`AffiliateBox`, `ProductCard`, `BestChoice`, `CTA`, `ComparisonTable`, `ProsCons`, `AdSlot`) render *inside* `.prose` when used as MDX components | Added Typography's own `not-prose` escape hatch to all 7 commerce components' root elements |
...
| 10 | Tag-count text on `/blog/tags` failed AA | `Badge` is intentionally theme-invariant (always a light chip) but this one usage nested a theme-*aware* muted text color inside it, which assumed a dark background that was never actually there | Switched to a fixed `--color-neutral-600`, matching Badge's own always-light design |
